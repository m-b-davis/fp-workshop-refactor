import { apiClient } from './solution.api';
...
describe.only('api tests', () => {
  ...
  test('get returns correct values', done => {
    apiClient.get(`${url}/users`).then(({ response }) => {
  ...
  test('addUser returns correct values', done => {
    apiClient.addUser({ username: 'foo' }).then(({ response }) => {
